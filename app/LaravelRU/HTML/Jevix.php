 ...
 * @version 1.01
 */
class Jevix implements HtmlPurifier
{
    ...
    public $dotes = "…";
    public $nl = "\r\n";
    public $defaultTagParamRules = ['href' => '#link', 'src' => '#image', 'width' => '#int', 'height' => '#int', 'text' => '#text', 'title' => '#text'];
    ...
    protected $autoReplaceAfter; // Автозамена после обработки
    ...
    /**
     * КОНФИГУРАЦИЯ: Не короткие теги которые не нужно удалять с пустым содержанием,
     * например, <param name="code" value="die!"></param>
     *
     * @param array|string $tags тег(и)
     */
    ...
    /**
     * КОНФИГУРАЦИЯ: Теги внутри который не нужна авто-расстановка <br/>,
     * например, <ul></ul> и <ol></ol>
     *
     * @param array|string $tags тег(и)
     */
     ...
     * @param boolean $isContainerOnly тег является только контейнером других тегов
     * и не может содержать текст
     * @param boolean $isChildOnly вложенные теги не могут присутствовать нигде кроме
     * указанного тега
     * @throws Exception
     */
    ...
    /**
     * Автозамена после обработки
     *
     * @param array $from с
     * @param array $to на
     */
    public function cfgSetAutoReplaceAfter($from, $to)
    {
        $this->autoReplaceAfter = ['from' => $from, 'to' => $to];
    }
    ...
    public function parse($text)
    {
        ...
        if ( ! empty($this->autoReplaceAfter))
        {
            $content = str_ireplace($this->autoReplaceAfter['from'], $this->autoReplaceAfter['to'], $content);
        }
...

    {
        ...
        while ($this->curChClass)
        {
            ...
            $tmp_content .= isset($this->entities2[$this->curCh])
                ? $this->entities2[$this->curCh]
                : $this->curCh;
            $this->getCh();
        }
        ...
        while ($this->curChClass)
        {
            ...
            $content .= isset($this->entities2[$this->curCh])
                ? $this->entities2[$this->curCh]
                : $this->curCh;
            $this->getCh();
        }
    ...
    protected function makeTag($tag, $params, $content, $short, $parentTag = null)
    {
        ...
        foreach ($params as $param => $value)
        {
            ...
            $paramAllowedValues = isset($tagRules[self::TR_PARAM_ALLOWED][$param])
                ? $tagRules[self::TR_PARAM_ALLOWED][$param]
                : false;
        ...
        $requiredParams = isset($tagRules[self::TR_PARAM_REQUIRED])
            ? array_keys($tagRules[self::TR_PARAM_REQUIRED])
            : [];
        if ($requiredParams)
        {
    ...
    protected function eror($message)
    {
        ...
        $this->errors[] = ['message' => $message, 'pos' => $this->curPos, 'ch' => $this->curCh, 'line' => 0, 'str' => $str,];
    }
}
